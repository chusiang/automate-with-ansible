...
[command][command_module] module 是個可以讓我們在遠端機器上執行指令的指令模組 (Commands Modules)，剛入門會用 module 不多？只要 cli 會通的都可以用它處理。但它不支援變數 (variables) 和 `<`, `>`, `|`, `;`, `&` 等運算，若有這類需求請改用 [shell][shell_module] module。
