...
在上一章「[11. 怎麼看 Ansible Modules 文件？](11.how-to-see-the-ansible-module-document.md)」學會怎麼看文件以後，接著凍仁將介紹 8 個較常用的模組 (Modules)。
...
[apt][apt_module] module 是給 Debian, Ubuntu 等作業系統 (OS) 使用的套件模組 (Packaging Modules)，我們可以透過它來管理 apt 套件。其類似的 Linux 指令有 `apt`, `apt-get`, `aptitude`, `dpkg`。
...
[command][command_module] module 是個可以讓我們在遠端機器上執行指令的指令模組 (Commands Modules)，剛入門會用 module 不多？只要 cli 會通的都可以用它處理。但它不支援變數 (variables) 和 `<`, `>`, `|`, `;`, `&` 等運算，若有這類需求請改用 [shell module][shell_module]。
...
[copy][copy_module] module 是個讓我們從本地複製檔案到遠端的檔案模組 (Files Modules)，若有使用變數需求者，可藉由 [template][template_module] module，這部份凍仁會在第 14 章提到。其類似的 Linux 指令為 `scp`。

1. 複製 ssh public key 到遠端 (`chmod 644 /target/file`)。

        # copy ssh public key to remote node
        - copy:
...
1. 複製 nginx vhost 設定檔到遠端，並備份原有的檔案。
        ...
        # copy nginx vhost to remote node and backup the original configure
        - copy:
...
[file][file_module] module 是個讓我們在遠端建立和刪除檔案 (file)、目錄 (directory)、軟連結 (symlinks) 的檔案模組 (Files Modules)。其類似的 Linux 指令為 `chown`, `chown`, `ln`, `mkdir`, `touch`。

1. 建立檔案 (`touch`)，並設定檔案權限為 644。

        - name: touch a file, and set the permissions (equivalent to 0644)
          file:
            path: /etc/motd
            state: touch
            mode: "u=rw,g=r,o=r"
...
1. 建立目錄 (`mkdir`)，並設定檔案擁有者為 docker。
        ...
        - name: create a directory, and set the owner and permissions
          file:
...
[file_module]: http://docs.ansible.com/ansible/file_module.html
...
[shell][shell_module]  module 是個可以讓我們在遠端機器上用 `/bin/sh` 執行指令的指令模組 (Commands Modules)，支援變數 (variables) 和 `<`, `>`, `|`, `;`, `&` 等運算。
...
[stat][stat_module] module 是個用來檢查檔案狀態的檔案模組 (Files Modules)。其類似的 Linux 指令為 `stat`。
...
[stat_module]: http://docs.ansible.com/ansible/stat_module.html


### 後語

以上為凍仁較常用的 8 個 Modules，若不知該從哪裡下手，不妨先玩玩以上的 Modules 吧！
